...
class Mesa extends Component {
    ...
    classSize() {
      return {
        height:this.props.mesaInfo.height,
        width:this.props.mesaInfo.width,
        transform: "rotate(" + this.props.mesaInfo.giro + ")"
        }
    };
    ...
    render() {
        return (
            ...
            <div className='mesa-wrapper'  style={this.classPositionSize()}>
                <input clickeable="true" onClick={(e) => this.props.toggleLayout(e, this.props.mesaInfo.id)} className={'btn btn-info py-0 px-0'} style={this.classSize()} type="button" value={this.props.mesaInfo.nombre} />
            </div>
        )
    }
}
